...
public class ValidationItemControllerV2 {
	...
	public void init(WebDataBinder dataBinder) {
		log.info("initbinder init method start");
	...
	public String addItemV1(@ModelAttribute Item item
		...
		if (item.getQuantity() == null || item.getQuantity() >= 9999) {
			bindingResult.addError(
				new FieldError("item", "quantity", "수량은 최대 9.999 까지 허용 됩니다."));
		}
	...
	public String addItemV2(@ModelAttribute Item item
		...
		if (item.getQuantity() == null || item.getQuantity() >= 9999) {
			bindingResult.addError(
				new FieldError("item", "quantity", item.getQuantity(), false, null, null, "수량은 최대 9.999 까지 허용 됩니다."));
		}
		...
		if (item.getPrice() == null || item.getPrice() < 1000 || item.getPrice() > 1000000) {
			bindingResult.addError(
				new FieldError("item", "price", item.getPrice(), false, new String[] {"range.item.price"},
					new Object[] {1000, 1000000}, "가격은 1,000 ~ 1,000,000 까지 허용합니다."));
		}
		if (item.getQuantity() == null || item.getQuantity() >= 9999) {
			bindingResult.addError(
				new FieldError("item", "quantity", item.getQuantity(), false, new String[] {"max.item.quantity"},
					new Object[] {9999}, "수량은 최대 9.999 까지 허용 됩니다."));
		}
